...
- Development launcher: root-level `dev.py`; usage instructions live in `.codex/context/DEV_LAUNCHER.md`.
- Runtime logs for Frontend, API, Pre, Post, Solver, and Coordinator are exposed through the frontend Logs view. Backend services/workers write native local JSONL rotating files under `LOGS_FILES_ROOT`; `dev.py` also writes child stdout/stderr into `*-console.jsonl` files and prints only error-level child output to the terminal.
...
- Regeneration now creates a first `document_operations` row from the root `document` block with `operation_template_id = document_initial_data` and `operation_kind = billet`. Its `operation_parameters` uses nested namespaces that render as chained-dot parameters: `document_info`, `production_data`, `material`, `input_stock`, and `mesh`. The preprocessor maps this semantic row to the legacy billet/NewBillet compiler path.
...
- Surface preview meshes for Steps are generated only by the active Pre compiler. Initial billet meshes are generated by billet-type geometry code in `backend/app/services/preprocessor/geometry.py`; carried/deformed meshes still use the restored legacy Trimesh/STL mesh-state path in `backend/app/services/preprocessor/legacy_surface_mesh.py`. The compiler carries previous-row final meshes through billet, heating/furnace, upsetting, prolongation/radial/full-die, and cutting rows, and `backend/app/orchestration/runtime_backend.py` writes canonical binary PLY artifacts row-by-row under `TEMP_FILES_ROOT/runs/<document_version_id>/<execution_order>/surface/document_operation_<id>/`. `backend/app/services/preprocessor/surface_mesh.py` is the API payload/container plus PLY read/write bridge. Hidden geometry-JSON extrusion fallback is forbidden: if an artifact is missing or unreadable, the backend returns an explicit error and the UI shows that error. Durable artifact metadata is stored in `simulation_step_geometry_artifacts`, with compact compatibility references mirrored under `simulation_steps.calculations.surface_artifacts`; old DB `BYTEA` STL columns are still not restored.
...
- The Pre worker persists compiled output into `simulation_steps` row-by-row. At Pre start, sibling rows are reset to `calculations.preprocessor_status = pending`; each successful row is committed in its own short transaction with `preprocessor_status = compiled` before the next row starts compiling; if a later row fails, previously compiled rows remain visible in the Steps view and the failed row receives diagnostics.
- Worker processes are error-resilient at the job boundary. `WorkerProcess` treats ordinary `Exception` from job claim, job execution, stale-claim recovery, notification wait, or notification handling as recoverable worker-loop errors: it logs the exception, persists stage-specific failure state when a claimed job exists, backs off briefly where appropriate, and returns to waiting/claiming. `KeyboardInterrupt`, `SystemExit`, and other `BaseException` subclasses are not swallowed. Pre/Solver/Post workers override the job-error hook to mark the corresponding `DocumentVersion`, `SimulationStepStatus`, or `PostprocessingTask` failed so one bad job does not kill the long-running worker process.
- Pre has a development-only graceful reload runner at `python -m app.workers.pre_dev_reload`. It watches backend Python files, starts `app.workers.pre_worker` as a child process, sends the child a normal stop signal on source changes, waits for the current Pre job to finish, and only then starts a fresh Pre process. No Solver/Post reload runner is active.
...
- `MenuBar` includes explicit `Preprocessor` and `Postprocessor` result toggles near Save. Both toggles are off by default so document editing stays compact.
...
- `Steps`: no middle pane; selecting the tool opens the Pre troubleshooting inspector for the selected document and shows a narrow left step list; clicking the active `Steps` tool again hides only that left step list while keeping the main Steps inspector open
  ...
  - `Logs`: no middle pane; selecting the tool switches the main editor area to a local log viewer for Frontend, API, Pre, Post, Solver, and Coordinator logs
...
- steps main view: `SimulationStepsView` reading `GET /documents/{document_id}/simulation-steps` plus `GET /documents/{document_id}/blocks/root`; the API response is grouped by provenance into `simulation_step` from `simulation_steps`, runtime `simulation_step_status`, and API/log `diagnostics`; the selected step also lazy-loads matching Pre records from `GET /logs/pre/related`; the view shows a narrow hideable left step list with visual-only Heating/Deformation/Furnace/Operation title cards, selected-step diagnostics, shared-scale 2D overlays, and Three.js/WebGL 3D previews
  ...
  - logs main view: `LogsView` reading `GET /logs/services` and `GET /logs/{service}/tail`; selected-step troubleshooting can read structured matches from `GET /logs/{service}/related`
- `BlockEditor` renders a visual document hierarchy over the flat `document_blocks` linked list:
  ...
  - the `document` block groups setup fields into compact Notion/Word-like sections: `Production data`, `Material`, `Input stock size`, and `Mesh`; section titles use one visual tab and their parameter tables use a second deeper tab
  ...
  - when the `Preprocessor` result toggle is active, the editor renders one inline result panel under the active block or exactly one selected block; Operation/Furnace panels show rows generated directly from that block, while Deformation/Heating panels show rows from their direct child Operation/Furnace blocks
  - inline Pre results reuse the same Steps 2D/3D side-by-side preview components and load the same legacy surface endpoint; no inline fallback geometry is generated when artifacts are missing
  - when the `Postprocessor` result toggle is active, the editor shows an explicit unavailable placeholder until Post result payloads are migrated
...
- `Operations` main view characteristics:
  ...
  - the first row is the root Document-derived `document_initial_data` billet row, exposing `document_info.*`, `production_data.*`, `material.*`, `input_stock.*`, and `mesh.*` chips
...
- `Steps` main view characteristics:
  ...
  - the response is intentionally not an Operations/source-data join: default step rows are grouped as `simulation_step` (`simulation_steps`), `simulation_step_status` (runtime status table), and `diagnostics` (API/log context)
  - `diagnostics.related_log_query.search_terms` is the compact machine-readable context used for log navigation and future icon/diagram dashboards; it is API metadata, not persisted Pre output
  ...
  - step cards are nested under those title cards and show compact Pre-output chips from `simulation_steps.calculations` and `simulation_steps.pre_output`
  - the main list intentionally avoids a full raw DB-table layout
  - selecting a step card opens detail panels separated by provenance: `simulation_steps` typed columns, `pre_input`, `pre_output`, `calculations`, geometry metadata, runtime `simulation_step_status`, API response diagnostics, and related Pre log records
  - related Pre logs are fetched only for the selected step through `GET /logs/pre/related`, using `document_operation_id` as the primary match key and operation/template/source fields as supporting evidence; this keeps the hot step-list response light while preserving troubleshooting evidence
  ...
  - 3D visualization lazy-loads JSON API mesh payloads from `GET /documents/{document_id}/simulation-steps/{document_operation_id}/surface`; this endpoint reads Pre-generated canonical binary PLY artifacts with durable metadata in `simulation_step_geometry_artifacts`, with legacy JSON/STL artifact reading kept only as a transition fallback. The frontend renders these meshes with Three.js/WebGL, `OrbitControls`, translucent initial/final layers, and visual-only sharp-edge overlays from `THREE.EdgesGeometry` using a 30 degree normal-angle threshold. Steps 3D view state is remembered in frontend session memory while browsing steps, and a local reset-view icon restores the default fitted CAD-like view. If artifacts are absent or unreadable, the endpoint returns an explicit error and the frontend displays that error instead of synthesizing a local geometry fallback. Round cross-sections must come from the Pre mesh path, not from rectangular-prism approximation.
  - selected-step artifact files can be downloaded through `/documents/{document_id}/simulation-steps/{document_operation_id}/surface/artifacts/{initial|final}/{ply}`; `json`/`stl` downloads are legacy-only during transition.
