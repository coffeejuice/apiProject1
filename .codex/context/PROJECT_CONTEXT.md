...
- steps main view: `SimulationStepsView` reading `GET /documents/{document_id}/simulation-steps` plus `GET /documents/{document_id}/blocks/root`; the API response is grouped by provenance into `simulation_step` from `simulation_steps`, runtime `simulation_step_status`, and API/log `diagnostics`; the selected step also lazy-loads matching Pre records from `GET /logs/pre/related`; the view shows a narrow hideable left step list with visual-only Heating/Deformation/Furnace/Operation title cards, selected-step diagnostics, shared-scale 2D overlays, and outline-based 3D previews
  ...
  - logs main view: `LogsView` reading `GET /logs/services` and `GET /logs/{service}/tail`; selected-step troubleshooting can read structured matches from `GET /logs/{service}/related`; Solver is not included
