...
- `Steps` main view characteristics:
  ...
  - 3D visualization lazy-loads JSON triangular surface meshes from `GET /documents/{document_id}/simulation-steps/{document_operation_id}/surface`; this endpoint reads only legacy Trimesh JSON/STL artifacts written during Pre row compilation. The frontend renders these meshes with Three.js/WebGL, `OrbitControls`, translucent initial/final layers, and visual-only sharp-edge overlays from `THREE.EdgesGeometry` using a 30 degree normal-angle threshold. Steps 3D view state is remembered in frontend session memory while browsing steps, and a local reset-view icon restores the default fitted CAD-like view. If artifacts are absent, unreadable, or generated by a non-legacy source, the endpoint returns an explicit error and the frontend displays that error instead of synthesizing a local geometry fallback. Round cross-sections must come from the migrated old-project STL path, not from rectangular-prism approximation. Compact artifact references live in `simulation_steps.calculations.surface_artifacts`.
