...
## DB consistency snapshot (updated 2026-05-28)
- Alembic code state:
  - current head migration file: `0008_add_simulation_step_geometry_artifacts.py`
  - current head revision: `0008_step_geometry_artifacts`
...
- Current schema shape of note:
  ...
  - workflow runtime now has dedicated orchestration tables:
    - `document_operations`
    - `simulation_steps`
    - `simulation_step_geometry_artifacts`
...
- Verification status:
  ...
  - backend `.venv` `alembic current` should report `0008_step_geometry_artifacts (head)` after migration
  - backend `.venv` `alembic check` should report `No new upgrade operations detected.` after migration
  - expected active runtime tables include `document_blocks`, `document_operations`, `simulation_steps`, `simulation_step_geometry_artifacts`, `simulation_step_status`, and `postprocessing_tasks`
...
- `simulation_steps`
  ...
  - lazy surface endpoint: `GET /documents/{document_id}/simulation-steps/{document_operation_id}/surface`
    - returns JSON API triangular surface meshes (`vertices`, `faces`, bounds, area/volume summary)
    - reads canonical binary PLY artifacts generated row-by-row by the Pre compiler; initial billet meshes are generated by billet-type geometry code in `backend/app/services/preprocessor/geometry.py`, while deformation/carry-forward meshes still use the migrated mesh-state path in `backend/app/services/preprocessor/legacy_surface_mesh.py`; legacy JSON/STL artifact reading exists only as a transition fallback
    - never synthesizes fallback meshes from `simulation_steps` geometry JSON; missing, unreadable, or unsupported artifacts are reported as explicit errors
    - keeps mesh payloads out of the default `simulation_steps` list response
    - writes/read selected-step PLY files through `backend/app/services/preprocessor/surface_artifacts.py` under `TEMP_FILES_ROOT/runs/<document_version_id>/<execution_order>/surface/document_operation_<id>/`
    - records one canonical row per `{document_operation_id, initial|final, ply}` in `simulation_step_geometry_artifacts`, with compact compatibility metadata also mirrored under `simulation_steps.calculations.surface_artifacts`
  ...
  - lazy surface artifact endpoints:
    - `GET /documents/{document_id}/simulation-steps/{document_operation_id}/surface/artifacts/{initial|final}/{ply}`
    - serves cached canonical PLY artifact files for selected-step inspection/download; `json`/`stl` are accepted only for legacy artifact rows during transition
...
- `simulation_step_geometry_artifacts`
  - durable metadata table for selected-step 3D geometry artifacts
  - one canonical row per `document_operation_id`, geometry `kind` (`initial` or `final`), and `artifact_format = ply`; `json`/`stl` values are legacy-only during transition
  - `geometry_artifact_id`, `document_operation_id` (FK `simulation_steps.document_operation_id` cascade), `document_version_id` (FK `document_versions.document_version_id` cascade)
  - `source`, `relative_path`, `checksum_sha256`, `byte_size`
  - mesh summary columns: `vertex_count`, `face_count`, `cross_section_point_count`, `bounds`, `surface_area_mm2`, `volume_mm3`
  - `artifact_metadata`, `generated_at`, `created_at`, `updated_at`
  - unique on (`document_operation_id`, `kind`, `artifact_format`)
- `postprocessing_tasks`
