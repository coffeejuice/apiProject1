...
class Document(Base):
    ...
    source_document_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, ForeignKey("documents.document_id", ondelete="SET NULL"), nullable=True, default=None
    )
    editor_user_id: Mapped[Optional[int]] = mapped_column(
    ...
    first_block: Mapped[Optional["Block"]] = relationship(
        "Block",
        foreign_keys=[first_block_id],
        uselist=False,
        post_update=True,
    )
    ...
    edit_sessions: Mapped[List["DocumentEditSession"]] = relationship(
        "DocumentEditSession",
        back_populates="document",
        cascade="all, delete-orphan",
    )
...
class DocumentEditSession(Base):
    ...
    document_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("documents.document_id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    document: Mapped["Document"] = relationship("Document", back_populates="edit_sessions")
    editor: Mapped["User"] = relationship("User", back_populates="document_edit_sessions")
...
class DocumentVersion(Base):
    ...
    document_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, ForeignKey("documents.document_id", ondelete="RESTRICT"), nullable=True
    )
    parent_document_version_id: Mapped[Optional[int]] = mapped_column(
        BigInteger,
    ...
    simulation_server_id: Mapped[Optional[int]] = mapped_column(
        SmallInteger, ForeignKey("servers.id", onupdate="CASCADE", ondelete="SET DEFAULT"), nullable=True, default=None
    )
    ...
    document_priority_enum: Mapped[UserPriority] = mapped_column(
        SQLEnum(UserPriority, name="priority_enum"), nullable=False, default=UserPriority.normal
    )
    ...
    parent_version: Mapped[Optional["DocumentVersion"]] = relationship(
        "DocumentVersion", remote_side=[document_version_id]
    )
    simulation_server: Mapped[Optional["Server"]] = relationship(
        "Server", foreign_keys=[simulation_server_id]
    )
...
class DocumentACL(Base):
    ...
    __table_args__ = (Index("idx_document_user", "document_id", "user_id", unique=True),)
