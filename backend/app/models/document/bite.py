from sqlalchemy import String, BigInteger, SmallInteger, Float, DateTime, ForeignKey, Enum as SQLEnum, Integer, UniqueConstraint
...
class Bite(Base):
    ...
    bite_id: Mapped[int] = mapped_column(BigInteger, primary_key=True)
    bite_order: Mapped[Optional[int]] = mapped_column(SmallInteger, nullable=True)
    execution_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("server_pre_main.execution_id", ondelete="CASCADE", onupdate="CASCADE"), nullable=True)
    ...
    feed: Mapped[str] = mapped_column(String(63), default='', nullable=True)  # Compatibility with ForgeLab v.1
    ...
    simulation_duration_days: Mapped[float] = mapped_column(Float, default=0, nullable=True)
    ...
    __table_args__ = (
        UniqueConstraint('execution_id', 'bite_order', name='uk_post_bites_1'),
    )

    # *********************************** RELATIONSHIPS ******************************************

    execution: Mapped["ServerPreMain"] = relationship("ServerPreMain", back_populates="bites")
