...
from app.models import User, Document, Block, BlockType, Role
from app.schemas import ExportResponse, ImportRequest, DocumentResponse
...
@router.get("/documents/{document_id}/export", response_model=ExportResponse)
def export_document(
    document_id: int,
    ...
    markdown = export_to_markdown(db, document_id)
    return ExportResponse(markdown=markdown)

@router.post("/documents/import", response_model=DocumentResponse)
def import_document(
